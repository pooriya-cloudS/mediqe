...
class Schedule(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)  # Unique identifier for each schedule
    ...
    is_active = models.BooleanField(default=False)  # Indicates if the schedule is currently active


# Appointment model to represent a patient's appointment with a doctor
class Appointment(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)  # Unique identifier for each appointment
