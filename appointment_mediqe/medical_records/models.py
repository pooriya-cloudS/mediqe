...
class MedicalRecord(models.Model):
    ...
    )  # Unique identifier for each medical record
    ...
    is_sensitive = models.BooleanField(
        default=False
    )  # Indicates if the record contains sensitive information
...
class Prescription(models.Model):
    ...
    dosage = models.CharField(
        max_length=100
    )  # Dosage instructions (e.g., "2 tablets daily")
    instructions = (
        models.TextField()
    ...
    renewable = models.BooleanField(
        default=False
    )  # Whether the prescription can be renewed
    status = models.CharField(
...
class MedicalFile(models.Model):
    ...
    record = models.ForeignKey(
        MedicalRecord, on_delete=models.CASCADE, related_name="files"
    )  # Link to the related medical record
    ...
    type = models.CharField(
        max_length=50
    )  # Type of the file (e.g., "X-Ray", "Lab Report")
    ...
    is_private = models.BooleanField(
        default=False
    )  # Whether the file is private (restricted access)
